...
const CountryOfOriginScreen: React.FC = () => {
    ...
    const handleLongPress = (country: CountryOfOrigin) => {
        Alert.alert(
            'Land löschen',
            `Wollen Sie das Land "${country.originCountry}" löschen?`,
            [
                {
                    text: 'Nein',
                    style: 'cancel',
                ...
                {
                    text: 'Ja',
                    onPress: () => deleteCountry(country.id),
                },
            ],
            { cancelable: true }
        );
    ...
    const deleteCountry = async (id: string) => {
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`Failed to delete country with id ${id}:`, response.status, response.statusText, errorText);
    ...
    const renderCountry = ({ item }: { item: CountryOfOrigin }) => (
        <TouchableOpacity
            style={styles.card}
            onPress={() => handleSelectCountry(item.originCountry)}
